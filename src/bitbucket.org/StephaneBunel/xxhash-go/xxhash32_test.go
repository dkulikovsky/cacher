...
func Test_GoChecksum32(t *testing.T) {
	h32 := xxh.GoChecksum32(blob1)
	fmt.Printf("Checksum32(\"%v\") = 0x%08x\n", string(blob1), h32)
	if h32 != 0x1130e7d4 {
		t.Fail()
	}
...
func Test_Checksum32Seed(t *testing.T) {
	h32 := xxh.Checksum32Seed(blob1, 1471)
...
func Test_GoChecksum32Seed(t *testing.T) {
	h32 := xxh.GoChecksum32Seed(blob1, 1471)
	fmt.Printf("Checksum32Seed(\"%v\", 1471) = 0x%08x\n", string(blob1), h32)
	if h32 != 0xba59a258 {
		t.Fail()
	}
...
func Test_GoNew32(t *testing.T) {
	var digest = xxh.GoNew32()
	digest.Write(blob1)
	digest.Write(blob2)
	h32 := digest.Sum32()
	fmt.Printf("Sum32 = 0x%08x\n", h32)
...
func Test_GoNew32Seed(t *testing.T) {
	var digest = xxh.GoNew32Seed(1471)
	digest.Write(blob1)
	digest.Write(blob2)
	h32 := digest.Sum32()
	fmt.Printf("Sum32 = 0x%08x\n", h32)
...
func Benchmark_xxhash32(b *testing.B) {
	for i := 0; i < b.N; i++ {
		xxh.Checksum32(blob1)
	}
}

func Benchmark_goxxhash32(b *testing.B) {
	for i := 0; i < b.N; i++ {
		xxh.GoChecksum32(blob1)
	}
}
